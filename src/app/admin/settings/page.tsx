...
export default function AdminSettingsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await loadPolicies();
      toast("Platform policy saved successfully.");
    } catch (err) {
          ...
          <form onSubmit={handleSave} className="space-y-6">
            ...
            <div className="flex justify-end">
              <button
                type="submit"
                disabled={saving}
                className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-full bg-evo-red hover:bg-evo-red-hover text-white text-xs font-bold shadow-evo-button transition-all hover:scale-[1.02] disabled:opacity-60 w-full sm:w-auto"
