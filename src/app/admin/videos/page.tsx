...
import { api, Video, resolveMediaUrl } from "@/lib/api";
...
export default function AdminVideosPage() {
  ...
  const handleToggleHide = async (video: Video) => {
    ...
    try {
      ...
      toast(`Video "${video.title}" status changed to ${next}.`);
    } catch (e) {
      toast(e instanceof Error ? e.message : "Failed to update video", "error");
    }
  };
        ...
        <div className="bg-white rounded-3xl p-6 border border-gray-200/90 evo-card-shadow flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-medium focus:bg-white focus:border-gray-400 focus:ring-2 focus:ring-gray-200 focus:outline-none"
            />
          </div>

          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center bg-gray-100 rounded-xl p-1 text-xs font-bold text-gray-700 overflow-x-auto no-scrollbar">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50/80 border-b border-gray-100 text-[11px] font-bold uppercase tracking-wider text-gray-500">
                    <th className="py-4 px-6 min-w-[260px]">Video Media</th>
                    <th className="py-4 px-6 min-w-[140px]">Hub / Category</th>
                    <th className="py-4 px-6 min-w-[110px]">Published</th>
                    <th className="py-4 px-6 min-w-[110px]">Status</th>
                    <th className="py-4 px-6 min-w-[110px] text-right">Moderation</th>
                  </tr>
                </thead>
                    ...
                    <tr key={video.id} className="hover:bg-gray-50/60 transition-colors">
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3.5 min-w-[240px]">
                          ...
                          >
                            {resolveMediaUrl(video.thumbnailUrl || video.posterThumbnailKey) ? (
                              /* eslint-disable-next-line @next/next/no-img-element */
                              <img
                                src={resolveMediaUrl(video.thumbnailUrl || video.posterThumbnailKey)}
                                alt={video.title}
                                className="w-full h-full object-cover group-hover:scale-105 transition-transform"
                              />
                            ) : (
                              <div className="w-full h-full flex items-center justify-center bg-gray-800 text-gray-400 font-bold text-sm">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
            <div className="relative w-full max-w-3xl bg-gray-950 text-white rounded-3xl overflow-hidden border border-gray-800 shadow-2xl">
              ...
              <div className="relative aspect-video w-full bg-black overflow-hidden">
                {resolveMediaUrl(previewVideo.thumbnailUrl || previewVideo.posterThumbnailKey) ? (
                  /* eslint-disable-next-line @next/next/no-img-element */
                  <img
                    src={resolveMediaUrl(previewVideo.thumbnailUrl || previewVideo.posterThumbnailKey)}
                    alt={previewVideo.title}
                    className="w-full h-full object-cover opacity-80"
                  />
                ) : (
                  <div className="w-full h-full flex items-center justify-center bg-gray-900 text-gray-600 font-bold text-4xl">
                    {(previewVideo.title?.[0] ?? "E").toUpperCase()}
                  </div>
                )}
