  ...
  const handleSaveChannel = async (e: React.FormEvent) => {
    ...
    try {
      if (channel) {
        ...
        setChannel(updated);
        toast("Hub profile & branding updated successfully.");
      } else {
      ...
      toast(err instanceof Error ? err.message : "Failed to save Hub", "error");
    } finally {
      setSaving(false);
    }
  };
          ...
          <form onSubmit={handleSaveChannel} className="space-y-8">
            <div className="bg-white rounded-3xl p-7 border border-gray-200/90 evo-card-shadow space-y-6">
              ...
              <div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  <div className="relative w-20 h-20 rounded-2xl overflow-hidden bg-gray-900 border-2 border-white shadow-md shrink-0">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={logoUrl}
                      alt={name || "Hub avatar"}
                  ...
                  <input
                    ...
                    className="w-full sm:flex-1 px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-medium focus:bg-white focus:border-evo-red focus:outline-hidden"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <button
                type="submit"
                disabled={saving}
                className="inline-flex items-center justify-center gap-2 px-7 py-3 rounded-full bg-evo-red hover:bg-evo-red-hover text-white text-xs font-bold shadow-evo-button transition-all hover:scale-[1.02] disabled:opacity-60 w-full sm:w-auto"
