...
import { api, Video, Category, resolveMediaUrl } from "@/lib/api";
...
export default function CreatorVideosPage() {
  ...
  const handleCreateVideo = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsUploadOpen(false);
      toast(selectedFile ? `Video "${created.title}" published with file.` : `Video "${created.title}" saved.`);
    } catch (err) {
      toast(err instanceof Error ? err.message : "Failed to save video", "error");
    } finally {
      setSaving(false);
      setIsUploading(false);
    }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast(`Video "${deleteModalVideo.title}" was deleted.`);
      ...
      <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 bg-evo-red hover:bg-evo-red-hover text-white text-xs font-bold px-6 py-3 rounded-full shadow-evo-button transition-all hover:scale-[1.02] w-full sm:w-auto"
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50/80 border-b border-gray-100 text-[11px] font-bold uppercase tracking-wider text-gray-500">
                    <th className="py-4 px-6 min-w-[260px]">Video / Title</th>
                    <th className="py-4 px-6 min-w-[140px]">Genre Category</th>
                    <th className="py-4 px-6 min-w-[110px]">Uploaded</th>
                    <th className="py-4 px-6 min-w-[140px]">Status</th>
                    <th className="py-4 px-6 min-w-[80px] text-right">Actions</th>
                  </tr>
                </thead>
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3.5 min-w-[260px]">
                          <div className="relative w-20 h-12 rounded-xl bg-gray-900 overflow-hidden shrink-0 shadow-xs">
                            {resolveMediaUrl(vid.thumbnailUrl || vid.posterThumbnailKey) ? (
                              /* eslint-disable-next-line @next/next/no-img-element */
                              <img
                                src={resolveMediaUrl(vid.thumbnailUrl || vid.posterThumbnailKey)}
                                alt={vid.title}
                                className="w-full h-full object-cover"
                              />
                            ) : (
                              <div className="w-full h-full flex items-center justify-center bg-gray-800 text-gray-400 font-bold text-sm">
                                {(vid.title?.[0] ?? "E").toUpperCase()}
                              </div>
                            )}
                          </div>
