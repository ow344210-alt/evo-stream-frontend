...
import { Play, ChevronRight, Video as VideoIcon } from "lucide-react";
...
/**
 * Content Library — real, backend-driven discovery grid.
 * Fetches READY + PUBLISHED videos from the public feed and
  ...
  return (
    <section id="content-library" className="py-6 md:py-8 lg:py-12 bg-white relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Content: loading skeleton, then real backend card grid or empty state */}
        {loading && titles.length === 0 ? (
          ...
          <div className="py-12 text-center text-gray-500 bg-gray-50/80 rounded-3xl border border-gray-100 p-8">
            <div className="w-12 h-12 rounded-2xl bg-gray-100 text-gray-400 flex items-center justify-center mx-auto mb-3">
              <VideoIcon className="w-6 h-6" />
            </div>
            <p className="text-sm font-bold text-gray-800">
              {selectedCategory ? "No videos found in this category" : "No videos uploaded yet"}
            </p>
            <p className="text-xs text-gray-400 mt-1">
              Check back soon as creators publish new long-form content.
            </p>
          </div>
        ) : (
          <div
