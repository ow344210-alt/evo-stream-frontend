...
export function VideoPreviewModal({ videoId, onClose, onRequireAuth }: VideoPreviewModalProps) {
  ...
  // Escape closes the modal through the same clean path as the close button.
  useEffect(() => {
    if (!videoId) return;
  ...
  // Move focus to the close button on open and restore it on close.
  useEffect(() => {
    if (!videoId) return;
    const previous =
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Player area */}
        <div className="relative aspect-video w-full max-w-[min(106.6667vh,100%)] mx-auto bg-black overflow-hidden shrink-0">
          {state === "loading" && (
            <div
          ...
          {state === "error" && (
            <div
              className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black px-4 sm:px-6 text-center"
              role="alert"
            >
        ...
        {state === "loaded" && playback && (
          <div className="p-3 sm:p-4 lg:p-6 space-y-3 sm:space-y-4 overflow-y-auto flex-1 min-h-0">
            ...
            {playback.description && (
              <div>
                <p
                  className={`text-sm text-gray-300 leading-relaxed font-normal break-words ${
                ...
                {playback.description.length > 200 && (
                  <button
                    ...
                    className="mt-1 text-xs font-semibold text-gray-400 hover:text-white"
                  >
