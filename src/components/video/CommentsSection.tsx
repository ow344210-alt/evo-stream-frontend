...
function Avatar({ name, size = "w-8 h-8" }: { name: string; size?: string }) {
  return (
    <div
      className={`${size} rounded-full bg-gray-200 flex items-center justify-center text-gray-700 text-xs font-bold shrink-0`}
    >
      {(name?.[0] ?? "?").toUpperCase()}
    </div>
  );
    ...
    <li className="space-y-2.5">
          ...
          {editing ? (
            ...
            >
              <input
                ...
                className="flex-1 bg-transparent border-0 border-b border-gray-300 focus:border-evo-red px-0 py-1.5 text-sm text-gray-900 focus:outline-none focus:ring-0 rounded-none"
              />
              <button
                type="submit"
                disabled={busy || !draft.trim()}
                aria-label="Save edit"
                className="p-1.5 text-emerald-600 hover:bg-gray-100 rounded disabled:opacity-40"
              >
            ...
            {isOwner && (
              <>
                <button
                  ...
                  className="text-[11px] font-semibold text-gray-500 hover:text-gray-900 flex items-center gap-1 disabled:opacity-40"
                >
                ...
                <button
                  ...
                  className="text-[11px] font-semibold text-gray-500 hover:text-evo-red flex items-center gap-1 disabled:opacity-40"
                >
      ...
      {comment._count?.replies > 0 && (
        <button
          ...
          className="ml-11 text-[11px] font-semibold text-gray-500 hover:text-gray-900"
        >
          {repliesExpanded
            ? "Hide replies"
      ...
      {repliesExpanded && (
        <div className="ml-11 border-l border-gray-100 pl-3 space-y-2.5">
          {repliesLoading ? (
            <div className="space-y-2.5" aria-label="Loading replies">
              {[0, 1].map((i) => (
            ...
            replies.map((r) => (
              <div key={r.id} className="flex items-start gap-2.5">
                <Avatar name={r.user.name} size="w-7 h-7" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                  ...
                  {currentUserId != null && currentUserId === r.userId && (
                    <div className="mt-1 flex items-center gap-2">
                      <button
                        type="button"
                        disabled={busy}
                        onClick={() => onDeleteReply(r.id, comment.id)}
                        className="text-[11px] font-semibold text-gray-500 hover:text-evo-red flex items-center gap-1 disabled:opacity-40"
                      >
                        <Trash2 className="w-3 h-3" /> Delete
                      </button>
...
export function CommentsSection({
  ...
  // Track the logged-in user (from the stored session) to gate ownership UI.
  useEffect(() => {
    try {
  ...
  // Optimistic post: the comment renders immediately as a temp row, the draft
  // stays until the server confirms, then the temp is replaced by the real
  // comment. On failure the temp is removed and the draft kept.
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const temp: VideoComment = {
      ...
      user: { id: currentUserId ?? "temp", name: currentUserName || "You" },
      _count: { replies: 0 },
    };
    setPosting(true);
  ...
  if (loading && comments.length === 0) {
    return (
      <div className="space-y-4 pt-2" aria-label="Loading comments">
        {[0, 1, 2].map((i) => (
  ...
  if (error && comments.length === 0) {
    return <p className="py-6 text-sm text-gray-500">{error}</p>;
  }

  const composerActive = composerFocused || draft.trim().length > 0;

  return (
    <div className="space-y-6">
      {/* Composer — transparent underline input, actions snap right when active */}
      <form onSubmit={handleSubmit} className="flex items-start gap-3">
        <Avatar name={currentUserName} size="w-9 h-9" />
        <div className="flex-1 min-w-0">
          <input
            value={draft}
            onChange={(e) => setDraft(e.target.value)}
            ...
            aria-describedby="comments-limit"
            className="w-full bg-transparent border-0 border-b border-gray-300 focus:border-evo-red rounded-none px-0 py-2 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-0"
          />
          <span id="comments-limit" className="sr-only">
            Up to 2000 characters
          </span>
          <div
            className={`mt-2 flex items-center justify-end gap-2 transition-opacity ${
              composerActive ? "opacity-100" : "opacity-40"
            }`}
          >
            {composerActive && (
              <button
                ...
                className="px-3 py-1.5 rounded-full text-[13px] font-semibold text-gray-500 hover:text-gray-900"
              >
                Cancel
              </button>
            )}
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-4 h-9 rounded-full text-white text-[13px] font-semibold transition-colors disabled:cursor-not-allowed ${
                draft.trim()
                  ? "bg-evo-red hover:bg-evo-red-hover"
                  : "bg-gray-300"
              } disabled:opacity-40`}
            >
      ...
      {replyTarget && (
        <div className="rounded-xl border border-gray-200 bg-gray-50 p-3 space-y-2">
          ...
          <form onSubmit={handleSubmitReply} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
            <input
              value={replyDraft}
              ...
              className="flex-1 bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 placeholder-gray-400 focus:border-evo-red focus:outline-none"
            />
            <button
              type="submit"
              disabled={replyPosting || !replyDraft.trim()}
              aria-label="Post reply"
              className="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg bg-evo-red hover:bg-evo-red-hover text-white text-sm font-semibold disabled:opacity-40"
            >
              <Send className="w-4 h-4" />
              <span>Reply</span>
            </button>
      ...
      {comments.length === 0 ? (
        <div className="flex items-start gap-2 pl-12">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-gray-400">Be the first to comment.</p>
          </div>
        </div>
      ) : (
      ...
      {!loadedAll && comments.length > 0 && (
        <button
          ...
          className="w-full py-2.5 rounded-xl border border-gray-200 text-xs font-semibold text-gray-500 hover:bg-gray-50 disabled:opacity-40"
        >
