...
/**
 * A definitive authentication rejection (invalid/expired/revoked token or a
 * suspended account) means the persisted session is genuinely unusable.
 * Transient network / server failures must NOT be treated as a logout, so only
 * these statuses may destroy a persisted session.
 */
...
/**
 * A module-level single-flight access-token refresh.
 *
 * The backend rotates the refresh token on every use. Without this guard, two
 * simultaneous 401s would each call refresh(oldToken): the first succeeds and
 * rotates, the second is rejected (old token revoked) and would wrongly destroy
 * an otherwise valid session. All concurrent callers share one refresh Promise:
 * exactly one refresh request executes, everyone retries with the new token.
 */
  ...
  if (!refreshPromise) {
    refreshPromise = (async () => {
      try {
        ...
        if (!user) {
          throw new ApiError("No stored user to restore", 401);
        }
        storeTokens(refreshed.accessToken, refreshed.refreshToken, user);
 ...
 * refresh token exists, transparently refreshes (single-flight, shared by all
 * concurrent callers) and retries exactly once. Refresh-token failures that
 * are definitive authentication rejections clear the invalid session; transient
 * failures propagate without destroying persisted credentials and without
 * entering a 401 -> refresh -> retry loop.
 */
...
export const api = {
  ...
  recordView: (
    ...
    authenticatedRequest<{ recorded: boolean; reason?: string }>(
      `/videos/${videoId}/view`,
      {
        method: "POST",
        body: JSON.stringify({ sessionToken, watchDurationSeconds }),
      },
    ),
  recordProgress: (
    videoId: string,
